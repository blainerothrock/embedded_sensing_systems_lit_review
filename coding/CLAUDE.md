...
- **UI**: `view` (papers/matrix), `theme`, `sidebarOpen`, panel widths, `rightTab`
...
```
code (hierarchical)
  ├── top-level code = matrix column
  │     ├── sub-code = matrix dropdown option
  │     └── sub-code
  └── top-level code (no sub-codes = free text in matrix)

annotation (on PDF)
  ├── annotation_code → code (with relationship note)
  └── annotation_code → code

matrix_cell (paper × top-level code → inferred value)
  └── evidence: count of annotations tagged with this code or sub-codes
```
...
### Matrix
- `GET /api/matrix` — Papers x codes with cell values and evidence counts
- `POST /api/matrix/cell` — Save cell value
